...
subtitle: >
    A technical demonstration of the relationship between employee engagement
    and other measures in the People Survey.
body-classes: "insight-article"
author: Matt Kerlogue
date: 2026-01-17
date-format: "DD MMM YYYY"
image: ../../partials/plots/ee_framework.svg
...
From 2009 to 2016 the PDF-reports of People Survey results for organisations
and teams included a set of symbols that reported the outcome of 'driver
analysis' that was conducted on the relevant group of respondents for each
report.
...
In 2017, after I had stopped being directly involved in the People Survey, the
contractor instituted a methodological change to focus on question-level
correlation analysis. In 2020, a change in contractor for resulted in a change
in reporting options and platforms that significantly increased the level of
effort required to implement the historic approach to driver analysis.
...
csps_orgs <- arrow::read_parquet(here::here("../csps-data/data/02-organisations/csps_organisations_2009-2024_df950add.parquet"))
...
org_simple_model <- broom::glance(org_simple_reg)
...
tibble::tribble(
    ~statistic, ~value,
    "R<sup>2</sup>", scales::number(org_simple_model$r.squared, 0.001),
    "Adjusted R<sup>2</sup>", scales::number(org_simple_model$adj.r.squared, 0.001),
    "F-statistic", scales::comma(org_simple_model$statistic, 0.1),
    "p-value", scales::pvalue(org_simple_model$p.value),
    "df", paste(org_simple_model$df, scales::comma(org_simple_model$df.residual), sep = "; "),
    "Observations", scales::comma(nrow(org_ee_tm_scores))
) |>
    knitr::kable(
        col.names = NULL,
        align = "lr"
...
* Factor 7 has moderate correlations with the questions in the
...
inlinesvg::inline_svg_plot(
    loadings_plot, here::here("partials/plots/fa_loadings_plot.svg"),
...
represents. For the ease of interpretation in this analysis, I have given each
factor a single word label based on the structure of the factor model indicated
by the factor loadings in @fig-fa-loadings (although prefixed with `f1` for
factor 1, `f2` for factor 2 and so on).

@fig-tm-fa-corr shows the correlation matrix of the theme scores with the
factor scores, you can see in the upper triangle the high levels of correlation
most themes have with each other while in the lower triangle you can see the
very low level of correlation between each of the factor scores. Focussing on
the square showing the correlations of the theme scores against the factor
scores we see patterns that are similar to those in the factor loadings
(@fig-fa-loadings):
...
tm_fs_corr <- fa_scores_org |>
    dplyr::select(my_work:f9_feedback) |>
    as.matrix() |>
    Hmisc::rcorr()
...
tm_fs_cor_tidy <- broom::tidy(tm_fs_corr) |>
    dplyr::mutate(
        across(
            c(column1, column2),
            ~factor(.x, levels = names(fa_scores_org)[c(-1, -2, -3)])
        ),
        alpha = !(p.value < 0.05),
    )
...
tm_fs_plot <- ggplot(tm_fs_cor_tidy, aes(x = column2, y = column1)) +
    ...
    geom_vline(
        xintercept = 9.5, linewidth = 0.3, colour = "#6f6f6f", linetype = "dashed"
    ) +
    geom_text(
...
inlinesvg::inline_svg_plot(
    ...
    width = 800, height = 800, overwrite = TRUE,
...
```{r}
# fa_scores_org <- tibble::as_tibble(fa_model_org$scores, .name_repair = "unique_quiet") |>
#   dplyr::bind_cols(qm_input_org |> dplyr::select(uid_org_txt, year)) |>
#   dplyr::left_join(
#     csps_orgs |> 
...
[^fa]: FACTOR ANALYSIS EXPLANATION
